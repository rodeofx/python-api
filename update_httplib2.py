...
class Utilities:
    def download_archive(self, file_path, file_name):
        ...
        subprocess.check_output(
            [
                "curl",
                "-L",
                "https://github.com/httplib2/httplib2/archive/{}".format(file_name),
                "-o",
                file_path,
            ]
        )
    ...
    def remove_folder(self, path):
        ...
        print("Removing the folder {}".format(path))
        shutil.rmtree(path, ignore_errors=True)

    def git_remove(self, target):
        print("Removing {} in git.".format(target))
        try:
            subprocess.check_output(
                [
                    "git",
...
def main(temp_path, repo_root, version):
    ...
    root_folder = unzipped_folder / "httplib2-{}".format(version[1:])
    ...
    utilities.remove_folder("{}/test".format(python2_dir))
    utilities.remove_folder("{}/test".format(python3_dir))
